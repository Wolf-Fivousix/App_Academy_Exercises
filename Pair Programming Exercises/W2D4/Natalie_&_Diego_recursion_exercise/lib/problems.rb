...
def pow(base, exponent)
    return 1 if exponent == 0       # Corner case.
    return base if exponent == 1    # Base case.
...
# lucas_number(5)   # =>    11
# lucas_number(9)   # =>    76
def lucas_number(n) # = 2
    return 2 if n == 0
    return 1 if n == 1
    lucas_number(n-2) + lucas_number(n-1)
    # lucas_number(1) + lucas_number(2)
    #       1         + lucas_number(0) + lucas_number(1)
    #       1         +         2       +       1
    #   4 Result
end
...
def sum_array(array)
    ...
    sum_array([array[0]]) + sum_array(array[1..-1])
end
# sum_array([5, 2, 8]) => ???
# sum_array([2, 8])     => array.shift + 8
# sum_array([8])        => 8
